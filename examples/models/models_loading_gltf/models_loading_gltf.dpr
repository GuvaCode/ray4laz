...
begin
  ...
  // Load gltf model
  model := LoadModel(PAnsiChar(AnsiString(GetApplicationDirectory + 'resources/models/gltf/robot.glb')));

  // Load gltf model animations
  animsCount := 0;
  animIndex := 0;
  animCurrentFrame := 0;
  modelAnimations := LoadModelAnimations(PAnsiChar(AnsiString(GetApplicationDirectory + 'resources/models/gltf/robot.glb')), @animsCount);

  position := Vector3Create( 0.0, 0.0, 0.0 );    // Set model position

  if modelAnimations = nil then
    raise Exception.Create('Model animations is empty');
  ...
  while not WindowShouldClose() do
    begin
      ...
      animCurrentFrame := (animCurrentFrame + 1) mod Cardinal(anim.keyframeCount);
